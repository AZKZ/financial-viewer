...
import com.azkz.domain.incomestatement.items.monthly.impl.MonthlyExtraordinaryLossImpl
...
class MonthlyExtraordinaryLossTest {

    companion object {
        val YEAR_MONTH = YearMonth.parse("2020-01")
        val ACCOUNT_SUBCATEGORY = AccountSubcategory.EXTRAORDINARY_LOSS
    ...
    @Test
    fun `Test to create`() {
        val monthlyExtraordinaryLoss = MonthlyExtraordinaryLossImpl(YEAR_MONTH, BREAKDOWNS)
        assertEquals(monthlyExtraordinaryLoss.accountSubcategory, ACCOUNT_SUBCATEGORY)
        assertEquals(monthlyExtraordinaryLoss.yearMonth, YEAR_MONTH)
        assertEquals(monthlyExtraordinaryLoss.breakdowns, BREAKDOWNS)
    }

    @Test
    fun `Test to create by empty breakdowns`() {
        val monthlyExtraordinaryLoss = MonthlyExtraordinaryLossImpl(YEAR_MONTH, EMPTY_BREAKDOWNS)
        assertEquals(monthlyExtraordinaryLoss.breakdowns, EMPTY_BREAKDOWNS)
        assertEquals(monthlyExtraordinaryLoss.yearMonth, YEAR_MONTH)
        assertEquals(monthlyExtraordinaryLoss.breakdowns, EMPTY_BREAKDOWNS)
    }
